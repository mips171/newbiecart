...
func (cq *CompanyQuery) QueryOrders() *OrderQuery {
...
func (cq *CompanyQuery) WithOrders(opts ...func(*OrderQuery)) *CompanyQuery {
	query := (&OrderClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CompanyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Company, error) {
	...
	if query := cq.withOrders; query != nil {
		if err := cq.loadOrders(ctx, query, nodes,
			func(n *Company) { n.Edges.Orders = []*Order{} },
			func(n *Company, e *Order) { n.Edges.Orders = append(n.Edges.Orders, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CompanyQuery) loadOrders(ctx context.Context, query *OrderQuery, nodes []*Company, init func(*Company), assign func(*Company, *Order)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Company)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Order(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(company.OrdersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.company_orders
		if fk == nil {
			return fmt.Errorf(`foreign-key "company_orders" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "company_orders" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
