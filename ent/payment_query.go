...
func (pq *PaymentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Payment, error) {
	...
	if query := pq.withOrder; query != nil {
		if err := pq.loadOrder(ctx, query, nodes,
			func(n *Payment) { n.Edges.Order = []*Order{} },
			func(n *Payment, e *Order) { n.Edges.Order = append(n.Edges.Order, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PaymentQuery) loadOrder(ctx context.Context, query *OrderQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *Order)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(payment.OrderTable)
		s.Join(joinT).On(s.C(order.FieldID), joinT.C(payment.OrderPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(payment.OrderPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(payment.OrderPrimaryKey[1]))
