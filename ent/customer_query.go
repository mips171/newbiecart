...
func (cq *CustomerQuery) QueryCart() *CartQuery {
	query := (&CartClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CustomerQuery) WithCart(opts ...func(*CartQuery)) *CustomerQuery {
	query := (&CartClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CustomerQuery) loadOrders(ctx context.Context, query *OrderQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *Order)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(customer.OrdersTable)
		s.Join(joinT).On(s.C(order.FieldID), joinT.C(customer.OrdersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(customer.OrdersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(customer.OrdersPrimaryKey[0]))
...
func (cq *CustomerQuery) loadCart(ctx context.Context, query *CartQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *Cart)) error {
	...
	query.Where(predicate.Cart(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(customer.CartColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.customer_cart
		if fk == nil {
			return fmt.Errorf(`foreign-key "customer_cart" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
