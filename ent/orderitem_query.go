...
func (oiq *OrderItemQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *Product)) error {
	...
	byID := make(map[int]*OrderItem)
	nids := make(map[int]map[*OrderItem]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(orderitem.ProductTable)
		s.Join(joinT).On(s.C(product.FieldID), joinT.C(orderitem.ProductPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(orderitem.ProductPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(orderitem.ProductPrimaryKey[1]))
